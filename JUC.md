...
`ReentrantLock` 的公平锁实现**依赖于 AQS 提供的 FIFO 队列来记录和管理等待线程的顺序**。但更重要的是，它通过在尝试获取锁时**显式地调用 `hasQueuedPredecessors()` 方法来查询这个队列的状态**，并根据查询结果（即队列中是否有等待者）来决定当前线程是否有资格尝试获取锁，从而确保了锁的分配严格按照线程请求的FIFO顺序，实现了公平性。
...
通过一个共享的锁对象lock来控制两个线程的交替执行。一个线程负责打印奇数，另一个线程负责打印偶数，通过wait()和notify()方法来在两个线程之间实现顺序控制。当当前应该打印奇数时，偶数线程会进入等待状态，反之亦然。
...
| 特性         | 单例 (Singleton)                             | 多例 (Multiple Instances / Prototype) |
| :----------- | :------------------------------------------- | :------------------------------------ |
| **实例数量** | 整个应用中只有一个                           | 可以有任意多个，每次创建都是新的      |
| **状态共享** | 只有一个实例，状态是共享的（要小心线程安全） | 每个实例状态独立                      |
| **创建方式** | 通过特定的静态方法获取（如`getInstance()`）  | 通常通过 `new` 关键字直接创建         |
| **适用场景** | 全局配置、共享资源池、工具类等               | 大多数业务对象、数据对象等            |

“多例”这个词有时也可能特指一种不那么常见的设计模式叫 **Multiton（多例模式）**，它允许一个类有有限的、固定数量的实例，并且每个实例通过一个唯一的键（key）来获取。但这与我们通常说的与“单例”相对的“可以创建任意多个实例”的“多例”（即原型模式）是不同的。一般口语中，当我们说“单例还是多例”时，“多例”更多指的是后者——可以自由创建多个实例。

### 单例模式和多例模式

- **单例模式**：就是指确保一个类只有一个实例的那个设计模式。
- 多例模式：
  - 最常见的理解是与单例模式相对的、允许创建多个实例的类的设计（接近原型作用域的概念）。
  - 也可以特指**原型设计模式**（一种通过克隆创建对象的模式）。
  - 还可以特指不那么常见的**多例设计模式 (Multiton Pattern)**（管理有限数量的命名实例）。
