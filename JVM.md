...
假设 `Mode a = new Mode();` 这行代码是在一个方法体内部执行的（这是最常见的情况）：

- **`new Mode()` 创建的对象实例**：在 **堆（Heap）** 中。
- **引用变量 `a` 本身**：在 **栈（Stack）** 中。
- **指向关系**：栈中的引用变量 `a` **指向** 存放在堆中的 `Mode` 对象实例。

### 引用类型有哪些？有什么区别？

引用类型主要分为强软弱虚四种：

- 强引用指的就是代码中普遍存在的赋值方式，比如A a = new A()这种。强引用关联的对象，永远不会被GC回收。(只要这个强引用本身还存在并且对象是可达的（reachable from GC Roots）)(这个对象会不会被GC回收，取决于引用 `a` 的生命周期以及该对象是否还有其他强引用指向它)
...
3. **“静态变量在方法区？”**
   ...
   - **历史情况 (JDK 7及以前的HotSpot JVM的永久代)**：是的，可以这么说。
   - **当前情况 (JDK 8及以后的HotSpot JVM的元空间)**：**不准确。** 静态变量本身（即它们的值）现在存储在**堆内存**中，与它们所属的类的 `Class` 对象实例关联。而类的元数据（包括静态变量的定义信息）存储在元空间。

   **当前（以JDK 8+ 为主流的现在，2025年）的情况：**

   - **方法区/元空间 (Metaspace)**：主要存储类的元数据（字节码、字段和方法描述等）、运行时常量池中除字符串字面量和符号引用之外的部分。元空间使用的是本地内存（Native Memory），而不是JVM堆内存。
   - 堆 (Heap)：
     - 对象实例。
     - **字符串常量池** (String Table / String Pool)。
     - **静态变量的值** (associated with their `Class` objects on the heap)。
   - **方法区/元空间的垃圾回收**：主要是针对不再使用的类进行卸载（Class Unloading）和常量池中一些废弃常量的回收。